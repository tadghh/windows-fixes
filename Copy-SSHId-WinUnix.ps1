function Copy-SSHId {
    ...
    .EXAMPLE
    Copy-SSHId -WindowsRemote Administrator@windowsserver
    #>
    ...
    function Test-RemoteOS {
        ...
        $windowsTest = & ssh -p $Port "${Username}@${RemoteHost}" "echo %OS%" 2>$null
        if ($LASTEXITCODE -eq 0 -and $windowsTest -like "*Windows*") {
            return "Windows"
        }
        
        # Try Unix command
        $unixTest = & ssh -p $Port "${Username}@${RemoteHost}" "uname" 2>$null
        if ($LASTEXITCODE -eq 0 -and $unixTest) {
            return "Unix"
        }
        
        # Default to Unix if detection fails
        Write-Host "Could not detect remote OS, defaulting to Unix commands" -ForegroundColor Yellow
        return "Unix"
    }
    ...
    try {
        ...
        if ($WindowsRemote) {
			...
			Write-Host "Creating connection to determine OS (hint: use -WindowsRemote or -UnixRemote)" -ForegroundColor Yellow
        ...
        # Build command for target shell
        if ($remoteOS -eq "Windows") {
            # Windows cmd.exe - don't quote the key itself, just the path
            if ($Force) {
                $commandString = "echo $publicKey > `"%USERPROFILE%\.ssh\authorized_keys`""
            } else {
                $commandString = "echo $publicKey >> `"%USERPROFILE%\.ssh\authorized_keys`""
            }
        } else {
            ...
            if ($Force) {
                $remoteCommands += "echo '$escapedKey' > ~/.ssh/authorized_keys"
            } else {
                $remoteCommands += "echo '$escapedKey' >> ~/.ssh/authorized_keys"
            }
            $remoteCommands += @(
        ...
        Write-Host "  5. For Windows servers, ensure OpenSSH Server is installed" -ForegroundColor Yellow
    }
}
